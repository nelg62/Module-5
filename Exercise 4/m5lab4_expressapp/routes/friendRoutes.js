...
router.get("/filter", (req, res) => {
  ...
  if (filterName) {
    matchingFriends = matchingFriends.filter(
      (friends) =>
        friends.name.charAt(0).toLowerCase() == filterName.toLowerCase()
    );
  }
...
router.get("/info", (req, res) => {
  ...
  res.json(
    `${req.headers["user-agent"]},  ${req.headers["content-type"]}, ${req.headers["accept"]}`
  );
});
...
let friendWithID = friends.find(friend => friend.id === parseInt(friendId))
  // Modify this response with the matched friend, or a 404 if not found
  if (friendWithID) {
    res.json(friendWithID)
  }else {
  res.status(404).json({result: 'Finding friend with ID ' + friendId})
  }
})
...
router.put("/:id", (req, res) => {
  ...
  let findFriendPosition = friends.findIndex((friend) => friend.id === parseInt(friendId))
  console.log(findFriendPosition)
  // Replace the old friend data for friendId with the new data from updatedFriend

  // Modify this response with the updated friend, or a 404 if not found
 if (findFriendPosition === -1){
  res.status(404).json({result: 'No ID error'})
 }else {
  friends[findFriendPosition] = updatedFriend
  res.json({
